...
if not os.path.exists("resultslast"):
    os.makedirs("resultslast")
...
def evaluate_stacking(n_hidden):
    ...
    # اعتبارسنجی متقاطع
    cv_scores = cross_val_score(stacking, X_train_scaled, y_train, cv=2, scoring='accuracy', error_score='raise')
...
neuron_counts = [300,500,700,100]
results = []

for n in neuron_counts:
    try:
        acc, t, cv_mean, cv_std, report, conf_matrix = evaluate_stacking(n)
        results.append((n, acc, t, cv_mean, cv_std, report, conf_matrix))
        print(f"\nNeurons: {n}")
...
best_result = max(results, key=lambda x: x[1], default=(0, 0.0, 0.0, 0.0, 0.0, {}, np.zeros((6, 6))))
...
results_df = pd.DataFrame([
    {
        ...
        'CV_Std': cv_std,
        'Time': t
    } for n, acc, t, cv_mean, cv_std, _, _ in results
])
results_df.to_csv(os.path.join("results", "results.csv"), index=False)
print("\nResults saved to 'results/results.csv' and images saved to 'results/' folder.")