...
for name, model in models.items():
    ...
    for train_idx, val_idx in kfold.split(X_train_full, y_train_full):
        ...
        # آموزش مدل (بدون early stopping برای سازگاری با نسخه‌های جدید xgboost)
...
t_stat, p_value = ttest_rel(rf_acc, xgb_acc)
print(f"\nT-test between Random Forest and XGBoost: t-statistic={t_stat:.3f}, p-value={p_value:.3f}")
...
try:
    results_df.to_latex('model_results.tex', index=False, float_format="%.3f")
...
## Visualizations
- Feature importance and SHAP analysis saved as 'feature_importance.png' and 'shap_feature_importance.png'.
- ROC Curves saved as 'roc_curves.png'.
- Confusion matrices for top models saved as 'confusion_matrix_<model>.png'.

## Conclusion
The Random Forest and XGBoost models performed best, with the Voting Classifier providing robust results. Further analysis with larger datasets or additional features could enhance performance.
"""
